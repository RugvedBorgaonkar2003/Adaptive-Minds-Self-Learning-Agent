...
class ResearchAgent:
    ...
    def evaluate_source_quality(self, content: str, topic: str, level: str) -> bool:
        ...
        prompt = f"""You are a strict Source Quality Evaluator. 
Analyze the provided text snippet and determine if it is a high-quality, relevant source for learning about:
Topic: {topic}
Target Level: {level}

A high-quality source:
1. Actually discusses the topic significantly.
2. Contains substantive information (not just navigation links, cookie banners, or short SEO spam).
3. Roughly matches the target depth/level requested.

Return ONLY a valid JSON object with a single boolean key "is_valid".
...
"""
    ...
    async def conduct_research(self, topic: str, level: str, provided_urls: List[str] = None) -> str:
        """
